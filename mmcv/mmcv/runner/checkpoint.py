...
def load_checkpoint_cococlip(model,
                    filename,
                    map_location=None,
                    strict=False,
                    logger=None,
    ...
    for k, v in state_dict.items():
        if 'backbone.attnpool.positional_embedding' in k:
            # pdb.set_trace()
            if model.backbone.attnpool.positional_embedding.shape != v.shape:
                print(
                    f'Resize the pos_embed shape from {v.shape} to {model.backbone.attnpool.positional_embedding.shape}')
                cls_pos = v[0:1, :]
                if model.backbone.same_dim:
                    H = W = model.backbone.input_resolution // 32
                else:
                    H = model.backbone.nput_resolution // 32 #H is longer
                    W = model.backbone.short_dim // 32
                ...
                assert model.backbone.attnpool.positional_embedding.shape == v.shape
                state_dict[k] = v
