...
class TopDownGenerateTarget:
    ...
    def _msra_generate_target_down(self, cfg, joints_3d, joints_3d_visible, sigma):
        """Generate the target heatmap via "MSRA" approach.
    ...
    def __call__(self, results):
        ...
        if self.encoding == 'MSRA':
            if isinstance(self.sigma, list):
                ...
                target, target_weight = self._msra_generate_target(
                    ...
                    self.sigma)
                if self.downtarget:
                    target_down, target_down_weight = self._msra_generate_target_down(
                        results['ann_info'], joints_3d, joints_3d_visible,
                        self.sigma)
        ...
        if self.downtarget:
            results['target'] = target, target_down
            results['target_weight'] = target_weight, target_down_weight
        else:
            results['target'] = target
            results['target_weight'] = target_weight
        # results['target_down'] = target_down
        # results['target_down_weight'] = target_down_weight
